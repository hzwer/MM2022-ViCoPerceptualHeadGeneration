...
seg_net = U2NET(3,1) 
seg_net.load_state_dict(torch.load("/data/workspace/vico_workspace/vico_challenge_baseline/PIRender/u2net_human_seg.pth")) 
seg_net.cuda()
seg_net.eval()
...
parser.add_argument('--task', default='eval', required=True, type=str, help='speaker_test | listener_test | eval')
parser.add_argument('--output_dir', type=str)

args = parser.parse_args()

if args.task == 'speaker_test':
    csv_file = pd.read_csv("/data/workspace/vico_workspace/data/talking_head_test/test.csv")
elif args.task == 'listener_test':
    csv_file = pd.read_csv("/data/workspace/vico_workspace/data/listening_head_test/test.csv")
elif args.task == 'eval':
    csv_file = pd.read_csv("/data/workspace/vico_workspace/data/listening_head/eval.csv")
else:
    print("illegal task name")
    assert False
    ...
    with torch.no_grad():
        for video_index in tqdm(range(dataset.__len__())):
            ...
            for frame_index in tqdm(range(loop_length)):
                ...
                input_source1 = (input_source1 - down) / (up - down) * 2 - 1 # =>[-1, 1]
                a = (input_source1.cuda() + 1) / 2   
                output_dict1 = net_G(input_source1, target_semantic)
                output_dict1['fake_image'] = (output_dict1['fake_image'].clamp_(-1, 1) + 1) / 2 * (up - down) + down
                ...
                output_dict1['fake_image'] = output_dict1['fake_image'] * (1 - m) + input_source1 * m 

                pred_list.append(
                    output_dict1['fake_image'].cpu().clamp_(-1, 1)
                )
                if frame_index == 0:
                    mean = input_source1
                else:
                ...
                warp_images.append(
                    output_dict1['warp_image'].cpu().clamp_(-1, 1)
                    )                    
                gt_images.append(
